...
module.exports = function(router, userRepository){
    ...
    //return all user records if logged user has rights to do that
    router.register('user/get-all', function(req, next){
        userRepository.getAllUsers(req.session.passport.user.id_user_role, next);
    });
    //A new users are inserted to database from coma separated value format
    router.register('user/get/HR', function(req, next){
