...
module.exports = function(dbClient){
	...
	return{
        /**
         * Retrieve all tasks from table tasks if logged user is Administrator
         * @param userId - id of user
         * @param {function} next - callback function
         * @returns {undefined}
         */
         getAllTasks : function(userId, next){
            dbClient.queryAll("SELECT t.* FROM tasks t  \
                               JOIN user_roles ur ON ur.title='Administrator'\
                               JOIN users u ON u.id_user=$1 AND u.id_user_role = ur.id_user_role", [userId], next);
         },
