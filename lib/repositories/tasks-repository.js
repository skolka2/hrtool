...
module.exports = function(dbClient){
	...
	return{
         ...
         */
         ...
         getUserTasksCompleted : function(userId, next){
          dbClient.queryAll('SELECT t.*, u.email AS buddy_email, u.last_name AS buddy_last_name, u.first_name AS buddy_first_name \
              FROM tasks t \
              LEFT JOIN users u ON u.id_user= t.id_buddy  \
              WHERE t.id_user=$1 AND t.completed=TRUE', [userId], next);
         },

         getUserTasksNotCompleted : function(userId, next){
          dbClient.queryAll('SELECT t.*, u.email AS buddy_email, u.last_name AS buddy_last_name, u.first_name AS buddy_first_name \
              FROM tasks t \
              LEFT JOIN users u ON u.id_user= t.id_buddy  \
              WHERE t.id_user=$1 AND t.completed=FALSE', [userId], next);
         },
         ...
         getBuddyTasksCompleted : function(userId, next){      
          dbClient.queryAll('SELECT t.*, u.* \
            FROM tasks t \
            JOIN users u ON u.id_user = t.id_user \
            WHERE t.id_buddy = $1 AND t.completed=TRUE',[userId], next);
         },

         getBuddyTasksNotCompleted : function(userId, next){      
          dbClient.queryAll('SELECT t.*, u.* \
            FROM tasks t \
            JOIN users u ON u.id_user = t.id_user \
            WHERE t.id_buddy = $1 AND t.completed=FALSE',[userId], next);
         },
