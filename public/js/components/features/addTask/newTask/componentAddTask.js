(function() {
  var ComponentAddTask, ComponentBase, ComponentContentSwitcher, ComponentDropdown, ComponentFilter, ComponentFilterFormatter, ComponentLeft, ComponentRight, Const, Model, NotificationCenter, hrtool,
  ...
  ComponentAddTask = (function(_super) {
    ...
    function ComponentAddTask(_preselectedUserId) {
      ...
      this._tabbedAreaComponent = new ComponentContentSwitcher(['New task', 'Choose template'], [[this._leftComponent], [this._rightComponent]]);
    ...
    ComponentAddTask.prototype.createDom = function() {
      ...
      this._personWrapper = this.element.getElementsByClassName(ComponentAddTask.classes.PERSON_WRAPPER_CLASS)[0];
      ...
      bottomDiv = this.element.getElementsByClassName(ComponentAddTask.classes.BOTTOM_WRAPPER_CLASS)[0];
    ...
    ComponentAddTask.prototype.checkInputs = function(userStatus, taskStatus, dateFrom, length, selectedTab) {
      ...
      if (userStatus.department.id === -1) {
        this.addNotification(ComponentAddTask.messages.NO_DEPARTMENT, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
        this._componentFilter._dropdowns[0].setInvalidInputClass();
        ret = false;
      }
      if (userStatus.team.id === -1) {
        this.addNotification(ComponentAddTask.messages.NO_TEAM, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
      ...
      if (userStatus.user.id === -1) {
        this.addNotification(ComponentAddTask.messages.NO_USER, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
      ...
      if (this._buddyDropdown.selected.id === -1) {
        this.addNotification(ComponentAddTask.messages.NO_BUDDY, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
        this._buddyDropdown.setInvalidInputClass();
        ret = false;
      }
      if (userStatus.user.id !== -1 && (userStatus.user.id === this._buddyDropdown.selected.id)) {
        this.addNotification(ComponentAddTask.messages.SAME_BUDDY_AND_USER, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
        this._componentFilter._dropdowns[2].setInvalidInputClass();
        this._buddyDropdown.setInvalidInputClass();
        ret = false;
      }
      if (dateFrom.toString() === 'Invalid Date' || dateFrom.getTime() < today.getTime()) {
        this.addNotification(ComponentAddTask.messages.WRONG_DATE, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
        this.setInvalidInputClass(this._dateInput);
        ret = false;
      }
      if (!Number(length)) {
        this.addNotification(ComponentAddTask.messages.WRONG_TASK_LENGTH, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
        this.setInvalidInputClass(this._lengthInput);
        ret = false;
      }
      if (selectedTab === 0) {
        if (taskStatus.title === '') {
          this.addNotification(ComponentAddTask.messages.NO_TASK_TITLE, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
          this.setInvalidInputClass(this._leftComponent._title);
          ret = false;
        }
        if (taskStatus.description === '') {
          this.addNotification(ComponentAddTask.messages.NO_TASK_DESCRIPTION, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
          this.setInvalidInputClass(this._leftComponent._text);
          ret = false;
        }
        if (taskStatus.save_as_template && taskStatus.department_id === -1) {
          this.addNotification(ComponentAddTask.messages.NO_TASK_DEPARTMENT, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
          this._leftComponent._filter._dropdowns[0].setInvalidInputClass();
          ret = false;
        }
        if (taskStatus.save_as_template && taskStatus.department_id === -1) {
          this.addNotification(ComponentAddTask.messages.NO_TASK_TEAM, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
      ...
      if (selectedTab === 1) {
        if (taskStatus.task_template.id === -1) {
          this.addNotification(ComponentAddTask.messages.NO_TEMPLATE, ComponentAddTask.NOTIFICATION_DURATION, NotificationCenter.eventType.ERROR);
