  ...
  ComponentStatusBar = (function(_super) {
    ...
    ComponentStatusBar.prototype.onLoad = function(data) {
      ...
      if (data.length) {
        ...
        infoSpan = this.getElementByClass(ComponentStatusBar.classes.INFO_SPAN_CLASS);
        infoSpan.innerText = "" + this.data.userName + "  |  " + this.data.finishedTasks + "/" + this.data.allTasks + " finished, " + this.data.deadlineTasks + " after deadline...";
        finishedDiv = this.getElementByClass(ComponentStatusBar.classes.PROGRESS_DIV_FINISHED);
        finishedDiv.style.width = this.getFinishedPartWidth();
        deadlineDiv = this.getElementByClass(ComponentStatusBar.classes.PROGRESS_DIV_DEADLINE);
